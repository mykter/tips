...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode {
  ...
  if (self) 
  {
    ...
    resultsCache = [NSMutableArray array];
  }
...
{
    if (![resultsCache containsObject: [twoDResult text]])
    {
        [resultsCache addObject: [twoDResult text]];
...
{ 
  ...
  cropRect.origin.y = 0.0;
    ...
    //Override to make sure it's always trying to decode...
    decoding = YES;
    //make sure we keep trying to decode the image...
...
- (void)stopCapture 
{
    NSLog(@"ZXingWidgetController stopCapture called.");
